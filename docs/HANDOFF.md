...
## 二、算法改动：分段器 confirm_frames（2026-09-14）
...
> 本节是**唯一的路线图**。第八、九节是支撑它的实测证据，不重复列待办。

### ① 重跑 Iron Gate ← 最高优先级，阻塞一切速度工作
...
所以 C++ **分段器单模块**的数值正确性有验证，但**全链路 + ASan 没有**。

> 上次 17/34 的残差主因是**两端 OpenCV 版本不一致**（沙箱内不可消除）。
> 本地若两端用同一版 OpenCV，通过率有望自然回升 —— 这是最可能的解释。

**为什么它阻塞速度工作**：速度优化最终落地在 C++ 侧。闸门不绿意味着
两端一致性未经全链路证明，此时做 INT8/NPU 等于在未对齐的基座上加优化。

### ② 采集留出集 ← 「测得准」唯一的出路

当前 32/34、RMSE 0.0487 **全部是 dev-only 数字，没有任何独立验证**。
第八节记录的四个过拟合方案是活证据：它们在开发集上"全都有效"。

按 [HOLDOUT.md](HOLDOUT.md) 的覆盖要求采集，入库自检：

```bash
python scripts/holdout_intake.py --check
```

> ⚠️ **小铁片会触发已知缺陷**：`mpp = 0.45m / 框高` 中的盘径是**写死的**
> （`vbtcore/calibrator.py`）。留出集要求轻负荷含 10/15kg 小片（非 450mm），
> 届时 mpp 必然系统性错、速度随之错。**这是留出集上线前必须先处理的**，
> 否则留出集成绩会被这个已知问题污染，得不出有效结论。
...
**第三条 `30kg_1.03...` 是新发现的孤立异常**：组内速度衰减趋势的 Spearman
相关，22 条（n≥3）平均 **ρ=+0.86**，唯独这条 **ρ=−0.20**。
真值从 1.03 衰减到 0.65（掉 37%，典型疲劳组），我们却预测几乎恒定的 ~0.88。
计数是对的，但动态完全没跟上 —— 要么拍摄有独特问题，要么真值存疑。

> 🚫 **在人工确认前，不要改 `dataset_index.json` / `ground_truth.csv`。**
> 这属于数据集口径变更。SCOREBOARD 当前按**原真值**记分。

### ④ 全量验证 redet_every=2，再改默认值

8 条子集实测是**免费的 2.1x 加速**（RMSE 反而略降），但改默认值前
必须在全量 34 条上确认，并通过参考视频物理校验。详见第九节。

### ⑤ 闸门转绿后再进第二阶段

主线是 **INT8 量化 + NPU 委托**（推理占 97%，是唯一能打到 <20ms 的杠杆）。
NDK r25c 交叉编译 `libvbtcore.so` → 打包 APK → 真机验证。
`android/` 下 JNI 壳（`cpp/vbtcore_jni.cpp`，48 行）与 CMakeLists 已就位。

> ⚠️ **原计划的 `stride=15` 已被实测否决**，实际可用上限 2~3。详见 9.2。
...
完整清单见 [TECH_ROUTE.md](TECH_ROUTE.md)。七条硬约束摘要：
...
7. **不要只看整体 RMSE 判断改动好坏**。第八节四个方案整体 RMSE 都降了，
   但都是拿高速档过冲换低速档收益，参考视频一律劣化。
   **必须同时看：参考视频物理校验 + 分档 bias + 配对 rep 数**（配对数下滑说明
   rep 边界对不齐，而 RMSE 只在配得上的 rep 里算，会掩盖这个损失）。
...
| `scripts/dump_all_trajectories.py` | 导出帧级轨迹缓存，供分段器亚秒级迭代 |
| `scripts/tune_segmenter.py` | 基于缓存的评估器（`--compare` / `--bias-report`） |
| `scripts/diagnose_missed_reps.py` | 少计视频三层帧级诊断 |
| `vbtcore-cpp/tools/segmenter_parity.cpp` | Python/C++ 分段器对拍（不依赖 OpenCV/ONNX） |
| `validation/reports/trajectories/` | 轨迹缓存（33 CSV + manifest，2.1 MB） |
...
## 九、「测得快」实测证据：稀疏检测曲线与杠杆排序

### 9.1 稀疏检测 `redet_every` 实测曲线

`vbtcore/pipeline.py` 已有 `redet_every` 参数（逐帧跟踪不变，只稀疏 YOLO 校正）。
8 条随机子集（seed=0）实测：

| redet_every | 计数 | RMSE | 配对 n | 耗时 | 加速 |
| --- | --- | --- | --- | --- | --- |
| 1（当前） | 8/8 | 0.0361 | 32 | 694s | 1.0x |
| **2** | **8/8** | **0.0343** | **32** | **336s** | **2.1x** |
| 3 | 8/8 | 0.0395 | 28 | 245s | 2.8x |
| 5 | 8/8 | 0.0411 | 27 | 158s | 4.4x |

**`redet_every=2` 是免费的 2.1 倍加速** —— RMSE 不升反降，配对数不变。
从 3 开始配对数下滑（32→28→27），说明部分 rep 边界已经对不齐，
RMSE 的"温和上升"掩盖了这个损失，**不能只看 RMSE 选参数**。

### 9.2 ⚠️ `redet_every=15` 精度崩溃（原第二阶段计划需修正）

原计划「每 15 帧稀疏 YOLO」实测结果：

| 视频 | 真值 | redet=1 | redet=15 |
| --- | --- | --- | --- |
| `110kg_0.71_0.73` | [0.71, 0.73] | [0.705, 0.762] | **[0.54, 0.49]** |
| `140kg_0.41` | [0.41] | [0.396] | **[0.284, 0.463]** 多切一个 |
| `80kg_0.88...` | 4 个 rep | 4 个 ✅ | **7 个 rep** ❌ |
| `30kg_1.03...` | [1.03,0.89,0.76,0.65] | [0.87,0.898,0.891,0.88] | [0.755,0.995,0.998,0.922] |

15 帧间隔下跟踪漂移已无法被校正，rep 被切碎。
**结论：第二阶段的 `stride=15` 目标不可直接采用，实际可用上限在 2~3。**

### 9.3 速度杠杆的正确优先级

单帧成本实测：ONNX 推理 **30.9ms**（2线程）/ 52.8ms（1线程），
预处理 0.8ms、LK 光流 0.67ms、分段器整条 0.3ms —— **推理占 97%**。

因此优先级应是：

1. **INT8 量化 + NPU 委托**（NNAPI/QNN）—— 唯一能把 30.9ms 打到 <20ms 的杠杆，
   且不牺牲精度。这是第二阶段的主线。
