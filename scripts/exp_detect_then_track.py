...
class DetectFitTracker:
    ...
    def _yolo_detect(self, frame) -> list[dict]:
        ...
        for d in dets:
            if rotated:
                cx_o, cy_o = rotate_coord_back(
                    d["cx"], d["cy"], scale, yo, xo, f_h, f_w
                )
            else:
    ...
    def _ncc_fit(self, frame, tpl, pred_cx, pred_cy):
        ...
        if (
            win_s.shape[0] < tpl["patch"].shape[0]
    ...
    def process(self, video_path: str) -> dict:
        ...
        while cap.isOpened():
            ...
            predict = True
            run_yolo = (
                self.redet_every is not None and n_frames % self.redet_every == 0
            ) or missing_run > 8
            ...
            if run_yolo:
                ...
                if dets:
                    ...
                    if cands:
                        cands.sort(
                            key=lambda d: np.hypot(d["cx"] - cx, d["cy"] - pred_y)
                        )
                        # 身份核验：候选处 NCC 分数也要过关（模板存在且 NCC 之前未失效时）
                        for cand in cands[:2]:
                            if tpl is not None and missing_run <= 2:
                                s = self._ncc_score_at(
                                    frame, tpl, cand["cx"], cand["cy"]
                                )
    ...
    # ── rep 提取（所有模式共用）────────────────────────────
    def _extract_reps(
        self, y, fps, scale, elapsed, n_frames, n_yolo_calls, srcs, ncc_scores, boot_ok
    ):
        # 大 gap 前后分段：只保留最长连续段
        segs, start = [], 0
        ...
        segs = [
            (a, b) for a, b in segs if b - a > 30 and not np.isnan(y[a : b + 1]).any()
        ]
        if not segs:
            return {
        ...
        return {
            ...
            "mean_ncc": float(np.mean([s for s in ncc_scores if s > 0]))
            if any(ncc_scores)
            else 0,
            "boot_ok": boot_ok,
            "segment_frames": (a, b),
...
def main():
    ...
    for vid, note in VIDEOS:
        ...
        for mode, kw in modes.items():
            ...
            if r["status"] == "OK" and gt:
                ...
                ev_mid = MetricsEvaluator.evaluate_video(
                    vid, gt, [{"mcv": m} for m in pred_mid]
                )
                ev_mean = MetricsEvaluator.evaluate_video(
                    vid, gt, [{"mcv": m} for m in pred_mean]
                )
                row.update(
                    {
                        "n_pred": len(pred_mid),
                        "rmse_mid": round(ev_mid.rmse, 3)
                        if not np.isnan(ev_mid.rmse)
                        else None,
                        "rmse_mean": round(ev_mean.rmse, 3)
                        if not np.isnan(ev_mean.rmse)
                        else None,
                ...
                print(
                    f"  [{mode:<12}] {r['ms_per_frame']:6.1f} ms/帧  YOLO调用率 {row['yolo_ratio'] * 100:5.1f}%  "
                ...
                print(
                    f"  [{mode:<12}] {r.get('ms_per_frame', 0):6.1f} ms/帧  status={r['status']}"
                )
            all_rows.append(row)
    ...
    for mode in modes:
        ...
        if spd:
            print(
                f"  [{mode:<12}] 平均 {np.mean(spd):6.1f} ms/帧 | YOLO调用率 {np.mean(yolo) * 100:5.1f}% | "
