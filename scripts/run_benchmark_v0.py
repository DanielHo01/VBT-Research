...
def main():
    ...
    ap.add_argument(
        "--tag", default="v0", help="报告后缀：输出 BENCHMARK_{tag}.md/json"
    )
    ap.add_argument(
        "--only",
        default=None,
    ...
    ap.add_argument(
        "--engine",
        default="vbtcore v1 (detect->fit hybrid, M0)",
        help="报告中的引擎标识",
    )
    ap.add_argument(
        "--no-regrind", action="store_true", help="关闭 M1.5 底部重锚定（烧蚀实验）"
    )
    ap.add_argument(
        "--bench-dir",
    ...
    if not (bench / "dataset_index.json").exists():
        sys.exit(
            f"[错误] 找不到基准索引: {bench / 'dataset_index.json'}\n"
    ...
    print(
        f"共 {len(dataset)} 个视频 | 基准目录: {bench} | "
    ...
    for k, item in enumerate(dataset):
        ...
        r = analyze_video(vp, MODEL, detector=det, regrind_enabled=not args.no_regrind)
        row = {
            "video": vid,
        ...
        row["regrind"] = [
            r.diagnostics.get("n_regrind_snap", 0),
    ...
    report = {
        ...
        "regrind_totals": {"snap": rg_tot[0], "micro": rg_tot[1], "reject": rg_tot[2]},
        "rep_level": rep_level,
        "rep_pairs": rep_pairs,
        "rows": rows,
    }
    ...
    lines = [
        ...
        f"- regrind 触发统计(snap/微偏/拒绝): {rg_tot[0]}/{rg_tot[1]}/{rg_tot[2]}",
        "",
    ...
    print(
        f"计数通过: {count_ok}/{n} | 配对视频 RMSE 均值 {report['video_rmse_mean']} "
