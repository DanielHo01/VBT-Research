...
# 计数与 GT 不符的视频。
#
# 2026-09-14 更新：原列表 3 条已诊断完毕，结论分两类 ——
#   - 110kg_0.61_0.52_0.55_0.51_0.38：算法真 bug（分段器早产换向），
#     已由 confirm_frames=3 修复，现为 5/5，故从本列表移除。
#   - 两条 105kg：算法输出与「独立于流水线的复核方法」一致（模板匹配 /
#     颜色质心均得 3 和 4），指向数据问题而非漏检，保留在此供复核追溯。
#
# 注意：本工具的 --all-missed 会对每条视频跑 stride=1 全量检测（单条 55~105s）。
# 若只是想复查轨迹，直接读 validation/reports/diagnostics/*.csv，不要重跑。
MISSED = [
